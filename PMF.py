...
def PMF(R,N,M,K,lambdaU,lambdaV,R_test,ul,il):
    ...
    def costL(U,V):
        ...
        cost = 0.5*((R - Rx).power(2)).sum()+0.5*lambdaU*(np.linalg.norm(U,ord=2,axis=1)**2).sum()+0.5*lambdaV*(np.linalg.norm(V,ord=2,axis=1)**2).sum()
        return cost
    def gradient(U,V):
        dU = lambdaU*U
        tmp = U.dot(V.T)
